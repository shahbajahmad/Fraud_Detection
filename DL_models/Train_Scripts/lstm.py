# Script 3: LSTM Model for Fraud Detection
import pandas as pd
import numpy as np
...
def train_and_evaluate_lstm(model, name, X_train, y_train, X_test, y_test, tune=False):
    try:
        if tune:
...
# Use a smaller subset for LSTM to manage memory
subset_size = 20000
...
results = train_and_evaluate_lstm(lstm_model, "LSTM", X_train_lstm, y_train, X_test_lstm, y_test, tune=False)  # Set tune=True for tuning
accuracy = results[0] if results else None

# Plot model accuracy (single model)
if accuracy is not None:
    plt.figure(figsize=(6, 4))
    sns.barplot(x=["LSTM"], y=[accuracy])
